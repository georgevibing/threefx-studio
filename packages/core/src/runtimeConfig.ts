...
export function createWispySmokeRuntimeConfig(
  ...
  return {
    ...
    emitters: [
      {
        ...
        noiseStrength: numberValue(values, "sourceNoiseStrength", 1.2),
        ...
        velocity: vec3Value(values, "sourceVelocity", [0.02, 1.1, 0.01]),
      },
    ],
    fields: [
      {
        ...
        speed: numberValue(values, "detailSpeed", 0.9),
    ...
    obstacles: [],
    render: {
      absorption: numberValue(values, "absorption", 1.05),
      baseDensity: numberValue(values, "baseDensity", 1.55),
      blendMode,
      detailOctaves: Math.max(1, Math.min(5, intValue(values, "detailOctaves", 4))),
      detailScale: numberValue(values, "detailScale", 21),
      detailSpeed: numberValue(values, "detailSpeed", 0.9),
      detailStrength: numberValue(values, "detailStrength", 3.35),
    ...
    solver: {
      ...
      velocityDissipation: numberValue(values, "velocityDissipation", 0.006),
    },
    sourceGlow: {
      blendMode: "additive",
  ...
  const fields = nodes
    ...
    .map((node) => {
      ...
      return {
        ...
        speed: numberValue(values, "detailSpeed", 0.9),
        strength: node.type === "field.fbm" ? numberValue(values, "detailStrength", 3.35) : numberValue(values, "turbulence", 1.85),
