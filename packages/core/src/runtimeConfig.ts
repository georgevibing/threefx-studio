...
export function createWispySmokeRuntimeConfig(
  ...
  return {
    ...
    emitters: [
      {
        density: numberValue(values, "density", 0.85),
        falloff: numberValue(values, "sourceFalloff", 0.95),
        id: "emitter",
        lifetime: numberValue(values, "lifetime", 4.5),
        ...
        scale: vec3Value(values, "sourceScale", [1, 0.72, 1]),
        shape: "sphere",
        spawnRate: numberValue(values, "spawnRate", 1200),
        temperature: numberValue(values, "sourceTemperature", 1.1),
        velocity: vec3Value(values, "sourceVelocity", [0.02, 0.55, 0]),
      },
    ],
    fields: [
      {
        bands: intValue(values, "turbulenceBands", 4),
        curlStrength: numberValue(values, "curlStrength", 1.4),
        id: "curl_field",
        scale: numberValue(values, "detailScale", 22),
        speed: numberValue(values, "detailSpeed", 0.65),
        strength: numberValue(values, "turbulence", 1.65),
        type: "curl",
        vorticityConfinement: numberValue(values, "vorticityConfinement", 0.85),
      },
    ],
    forces: [
      {
        buoyantLift: numberValue(values, "buoyantLift", 2.2),
        id: "buoyancy",
        position: [0, 0, 0],
        radius: 1,
        riseSpeed: numberValue(values, "riseSpeed", 1.8),
        strength: 1,
        type: "buoyancy",
        wind: vec3Value(values, "wind", [0.1, 0.3, 0.05]),
      },
    ],
    obstacles: [],
    render: {
      absorption: numberValue(values, "absorption", 6),
      baseDensity: numberValue(values, "baseDensity", 1.8),
      blendMode,
      ...
      detailStrength: numberValue(values, "detailStrength", 3.5),
      ...
      scattering: numberValue(values, "scattering", 2.2),
      shadowQuality: intValue(values, "shadowQuality", 12),
      shadowStrength: numberValue(values, "shadowStrength", 1.25),
      smokeColor: colorValue(values, "color", "#c7d2d8"),
      softness: numberValue(values, "softness", 0.78),
    },
    solver: {
      advectionMode,
      backendMode,
      densityDissipation: numberValue(values, "densityDissipation", 0.04),
      diffusion: numberValue(values, "diffusion", 0.01),
      diffusionIterations: intValue(values, "diffusionIterations", 1),
      gridResolution,
      pressureIterations: intValue(values, "pressureIterations", 12),
      quality,
      seed: intValue(values, "seed", 1337),
      velocityDissipation: numberValue(values, "velocityDissipation", 0.02),
    },
    sourceGlow: {
      blendMode: "additive",
  ...
  const fields = nodes
    ...
    .map((node) => {
      ...
      return {
        ...
        curlStrength: numberValue(values, "curlStrength", 1.4),
        id: node.id,
        scale: node.type === "field.fbm" ? numberValue(values, "detailScale", 22) : numberValue(values, "detailScale", 22),
        speed: numberValue(values, "detailSpeed", 0.65),
        strength: node.type === "field.fbm" ? numberValue(values, "detailStrength", 3.5) : numberValue(values, "turbulence", 1.65),
  ...
  const forces = nodes
    ...
    .map((node) => {
      ...
      return {
        ...
        radius: numberValue(values, "vortexRadius", 0.55),
        riseSpeed: numberValue(values, "riseSpeed", 1.8),
        strength: type === "vortex" ? numberValue(values, "vortexStrength", 0.35) : 1,
        type,
        wind: vec3Value(values, "wind", [0.1, 0.3, 0.05]),
      } as const;
    });
