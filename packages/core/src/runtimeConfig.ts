...
export function createWispySmokeRuntimeConfig(
  ...
  return {
    ...
    emitters: [
      {
        ...
        noiseStrength: numberValue(values, "sourceNoiseStrength", 0.94),
        position: vec3Value(values, "sourcePosition", [0, 0.18, 0]),
        radius: numberValue(values, "radius", 0.32),
        scale: vec3Value(values, "sourceScale", [1.18, 0.38, 1.18]),
        shape: "sphere",
        spawnRate: numberValue(values, "spawnRate", 720),
        temperature: numberValue(values, "sourceTemperature", 1.2),
        velocity: vec3Value(values, "sourceVelocity", [0.018, 0.86, 0.01]),
      },
    ],
    fields: [
      {
        bands: intValue(values, "turbulenceBands", 4),
        curlStrength: numberValue(values, "curlStrength", 3.12),
        id: "curl_field",
        scale: numberValue(values, "detailScale", 18),
        speed: numberValue(values, "detailSpeed", 1.35),
        strength: numberValue(values, "turbulence", 2.32),
        type: "curl",
        vorticityConfinement: numberValue(values, "vorticityConfinement", 3.65),
      },
    ],
    forces: [
      {
        buoyantLift: numberValue(values, "buoyantLift", 2.75),
        id: "buoyancy",
        position: [0, 0, 0],
        radius: 1,
        riseSpeed: numberValue(values, "riseSpeed", 2.55),
        strength: 1,
        type: "buoyancy",
    ...
    obstacles: [],
    render: {
      absorption: numberValue(values, "absorption", 1.72),
      baseDensity: numberValue(values, "baseDensity", 2),
      blendMode,
      ...
      detailStrength: numberValue(values, "detailStrength", 4.65),
      ...
      scattering: numberValue(values, "scattering", 3),
    ...
    solver: {
      ...
      gridResolution,
      pressureIterations: intValue(values, "pressureIterations", 8),
      quality,
  ...
  const fields = nodes
    ...
    .map((node) => {
      ...
      return {
        ...
        speed: numberValue(values, "detailSpeed", 1.35),
        strength: node.type === "field.fbm" ? numberValue(values, "detailStrength", 4.65) : numberValue(values, "turbulence", 2.32),
