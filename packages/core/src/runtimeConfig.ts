...
export function createWispySmokeRuntimeConfig(
  ...
  return {
    ...
    emitters: [
      {
        density: numberValue(values, "density", 0.24),
        ...
        noiseStrength: numberValue(values, "sourceNoiseStrength", 0.9),
        position: vec3Value(values, "sourcePosition", [0, 0.68, 0]),
        radius: numberValue(values, "radius", 0.42),
        scale: vec3Value(values, "sourceScale", [0.9, 0.4, 0.9]),
        shape: "sphere",
        spawnRate: numberValue(values, "spawnRate", 760),
        temperature: numberValue(values, "sourceTemperature", 1.28),
        velocity: vec3Value(values, "sourceVelocity", [0.02, 0.75, 0.02]),
      },
    ],
    fields: [
      {
        bands: intValue(values, "turbulenceBands", 4),
        curlStrength: numberValue(values, "curlStrength", 5.55),
        id: "curl_field",
        scale: numberValue(values, "detailScale", 26),
        speed: numberValue(values, "detailSpeed", 1.18),
        strength: numberValue(values, "turbulence", 2.6),
        type: "curl",
        vorticityConfinement: numberValue(values, "vorticityConfinement", 7.1),
      },
    ],
    forces: [
      {
        buoyantLift: numberValue(values, "buoyantLift", 1.85),
        id: "buoyancy",
        position: [0, 0, 0],
        radius: 1,
      ...
      },
      {
        ...
        riseSpeed: numberValue(values, "riseSpeed", 1.65),
        strength: numberValue(values, "vortexStrength", 0.16),
        type: "vortex",
        wind: [0, 0, 0],
      },
    ],
    obstacles: [],
    render: {
      absorption: numberValue(values, "absorption", 0.9),
      baseDensity: numberValue(values, "baseDensity", 2.35),
      blendMode,
      detailOctaves: Math.max(1, Math.min(5, intValue(values, "detailOctaves", 4))),
      detailScale: numberValue(values, "detailScale", 26),
      detailSpeed: numberValue(values, "detailSpeed", 1.18),
      detailStrength: numberValue(values, "detailStrength", 5.65),
      opacity: numberValue(values, "opacity", 0.92),
      opacityRamp,
      plumeTaper: numberValue(values, "plumeTaper", 0.48),
      renderStepScale: numberValue(values, "renderStepScale", 1.7),
      scattering: numberValue(values, "scattering", 3.65),
      shadowQuality: intValue(values, "shadowQuality", 12),
      shadowStrength: numberValue(values, "shadowStrength", 1.35),
      smokeColor: colorValue(values, "color", "#aebfc8"),
      softness: numberValue(values, "softness", 0.82),
    },
    solver: {
      advectionMode,
      backendMode,
      densityDissipation: numberValue(values, "densityDissipation", 0.03),
  ...
  const fields = nodes
    ...
    .map((node) => {
      ...
      return {
        ...
        curlStrength: numberValue(values, "curlStrength", 5.55),
        id: node.id,
        scale: node.type === "field.fbm" ? numberValue(values, "detailScale", 26) : numberValue(values, "detailScale", 26),
        speed: numberValue(values, "detailSpeed", 1.18),
        strength: node.type === "field.fbm" ? numberValue(values, "detailStrength", 5.65) : numberValue(values, "turbulence", 2.6),
  ...
  const forces = nodes
    ...
    .map((node) => {
      ...
      return {
        ...
        radius: numberValue(values, "vortexRadius", 1.55),
        riseSpeed: numberValue(values, "riseSpeed", 1.65),
        strength: type === "vortex" ? numberValue(values, "vortexStrength", 0.16) : 1,
        type,
        wind: vec3Value(values, "wind", [0, 0, 0]),
      } as const;
    });
