...
export function createWispySmokeRuntimeConfig(
  ...
  const sourceGlowIntensity = numberValue(values, "sourceGlowIntensity", 0.35);
  ...
  return {
    ...
    emitters: [
      {
        density: numberValue(values, "density", 0.62),
        falloff: numberValue(values, "sourceFalloff", 0.82),
        id: "emitter",
        lifetime: numberValue(values, "lifetime", 4.2),
        noiseScale: numberValue(values, "sourceNoiseScale", 9),
        noiseStrength: numberValue(values, "sourceNoiseStrength", 0.45),
        position: vec3Value(values, "sourcePosition", [0, 0.04, 0]),
        radius: numberValue(values, "radius", 0.28),
        scale: vec3Value(values, "sourceScale", [1.35, 0.46, 1.35]),
        shape: "sphere",
        spawnRate: numberValue(values, "spawnRate", 560),
        temperature: numberValue(values, "sourceTemperature", 0.75),
        velocity: vec3Value(values, "sourceVelocity", [0.02, 0.2, 0.01]),
      },
    ],
    fields: [
      {
        bands: intValue(values, "turbulenceBands", 4),
        curlStrength: numberValue(values, "curlStrength", 2.45),
        id: "curl_field",
        scale: numberValue(values, "detailScale", 13),
        speed: numberValue(values, "detailSpeed", 1.1),
        strength: numberValue(values, "turbulence", 2.1),
        type: "curl",
        vorticityConfinement: numberValue(values, "vorticityConfinement", 2.8),
      },
    ],
    forces: [
      {
        buoyantLift: numberValue(values, "buoyantLift", 1.62),
        id: "buoyancy",
        position: [0, 0, 0],
        radius: 1,
        riseSpeed: numberValue(values, "riseSpeed", 1.12),
        strength: 1,
        type: "buoyancy",
        wind: vec3Value(values, "wind", [0.07, 0.035, 0.03]),
      },
      {
        buoyantLift: numberValue(values, "buoyantLift", 1.62),
        id: "vortex",
        ...
        riseSpeed: numberValue(values, "riseSpeed", 1.12),
        strength: numberValue(values, "vortexStrength", 0.42),
        type: "vortex",
        wind: [0, 0, 0],
      },
    ],
    obstacles: [],
    render: {
      absorption: numberValue(values, "absorption", 2.65),
      baseDensity: numberValue(values, "baseDensity", 1.08),
      blendMode,
      detailOctaves: Math.max(1, Math.min(5, intValue(values, "detailOctaves", 4))),
      detailScale: numberValue(values, "detailScale", 13),
      detailSpeed: numberValue(values, "detailSpeed", 1.1),
      detailStrength: numberValue(values, "detailStrength", 3.6),
      opacity: numberValue(values, "opacity", 0.76),
      opacityRamp,
      plumeTaper: numberValue(values, "plumeTaper", 0.9),
      renderStepScale: numberValue(values, "renderStepScale", 1.25),
      scattering: numberValue(values, "scattering", 2.08),
    ...
    solver: {
      advectionMode,
      backendMode,
      densityDissipation: numberValue(values, "densityDissipation", 0.13),
      ...
      velocityDissipation: numberValue(values, "velocityDissipation", 0.018),
    },
    sourceGlow: {
      blendMode: "additive",
      ...
      radius: numberValue(values, "sourceGlowRadius", 0.68),
  ...
  const fields = nodes
    ...
    .map((node) => {
      ...
      return {
        ...
        curlStrength: numberValue(values, "curlStrength", 2.45),
        id: node.id,
        scale: node.type === "field.fbm" ? numberValue(values, "detailScale", 13) : numberValue(values, "detailScale", 13),
        speed: numberValue(values, "detailSpeed", 1.1),
        strength: node.type === "field.fbm" ? numberValue(values, "detailStrength", 3.6) : numberValue(values, "turbulence", 2.1),
  ...
  const forces = nodes
    ...
    .map((node) => {
      ...
      return {
        ...
        radius: numberValue(values, "vortexRadius", 1.15),
        riseSpeed: numberValue(values, "riseSpeed", 1.12),
        strength: type === "vortex" ? numberValue(values, "vortexStrength", 0.42) : 1,
        type,
        wind: vec3Value(values, "wind", [0.07, 0.035, 0.03]),
      } as const;
    });
