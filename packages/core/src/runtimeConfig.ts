...
export function createWispySmokeRuntimeConfig(
  ...
  return {
    ...
    emitters: [
      {
        ...
        id: "emitter",
        lifetime: numberValue(values, "lifetime", 7.1),
        ...
        scale: vec3Value(values, "sourceScale", [1.06, 0.5, 1.06]),
        shape: "sphere",
        spawnRate: numberValue(values, "spawnRate", 820),
        temperature: numberValue(values, "sourceTemperature", 1.2),
        velocity: vec3Value(values, "sourceVelocity", [0.055, 0.82, 0.035]),
      },
    ],
    fields: [
      {
    ...
    forces: [
      {
        buoyantLift: numberValue(values, "buoyantLift", 3.15),
        id: "buoyancy",
        position: [0, 0, 0],
        radius: 1,
        ...
        strength: 1,
        type: "buoyancy",
        wind: vec3Value(values, "wind", [0.16, 0.02, 0.08]),
      },
      {
        buoyantLift: numberValue(values, "buoyantLift", 3.15),
        id: "vortex",
        ...
        riseSpeed: numberValue(values, "riseSpeed", 2.55),
        strength: numberValue(values, "vortexStrength", 0.72),
        type: "vortex",
        wind: [0, 0, 0],
      },
    ],
    obstacles: [],
    render: {
      absorption: numberValue(values, "absorption", 1.95),
      ...
      detailStrength: numberValue(values, "detailStrength", 4.35),
      opacity: numberValue(values, "opacity", 0.96),
      opacityRamp,
      plumeTaper: numberValue(values, "plumeTaper", 0.22),
    ...
    solver: {
      ...
      velocityDissipation: numberValue(values, "velocityDissipation", 0.01),
    },
    sourceGlow: {
      blendMode: "additive",
  ...
  const fields = nodes
    ...
    .map((node) => {
      ...
      return {
        ...
        speed: numberValue(values, "detailSpeed", 1.35),
        strength: node.type === "field.fbm" ? numberValue(values, "detailStrength", 4.35) : numberValue(values, "turbulence", 2.85),
  ...
  const forces = nodes
    ...
    .map((node) => {
      ...
      return {
        ...
        radius: numberValue(values, "vortexRadius", 1.42),
        riseSpeed: numberValue(values, "riseSpeed", 2.55),
        strength: type === "vortex" ? numberValue(values, "vortexStrength", 0.72) : 1,
        type,
        wind: vec3Value(values, "wind", [0.16, 0.02, 0.08]),
      } as const;
    });
