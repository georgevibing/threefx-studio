...
export function createWispySmokeRuntimeConfig(
  ...
  return {
    ...
    emitters: [
      {
        ...
        shape: "sphere",
        spawnRate: numberValue(values, "spawnRate", 960),
        temperature: numberValue(values, "sourceTemperature", 1.28),
        velocity: vec3Value(values, "sourceVelocity", [0.12, 1.28, 0.05]),
      },
    ],
    fields: [
      {
    ...
    forces: [
      {
        ...
        strength: 1,
        type: "buoyancy",
        wind: vec3Value(values, "wind", [0.08, 0.02, 0.04]),
      },
      {
        buoyantLift: numberValue(values, "buoyantLift", 2.2),
        id: "vortex",
        ...
        riseSpeed: numberValue(values, "riseSpeed", 1.9),
        strength: numberValue(values, "vortexStrength", 0.34),
        type: "vortex",
        wind: [0, 0, 0],
      },
    ],
    obstacles: [],
    render: {
      absorption: numberValue(values, "absorption", 0.84),
      ...
      scattering: numberValue(values, "scattering", 3.05),
  ...
  const fields = nodes
    ...
    .map((node) => {
      ...
      return {
        ...
        speed: numberValue(values, "detailSpeed", 0.9),
        strength: node.type === "field.fbm" ? numberValue(values, "detailStrength", 4.8) : numberValue(values, "turbulence", 2.45),
  ...
  const forces = nodes
    ...
    .map((node) => {
      ...
      return {
        ...
        radius: numberValue(values, "vortexRadius", 1.55),
        riseSpeed: numberValue(values, "riseSpeed", 1.9),
        strength: type === "vortex" ? numberValue(values, "vortexStrength", 0.34) : 1,
        type,
        wind: vec3Value(values, "wind", [0.08, 0.02, 0.04]),
      } as const;
    });
