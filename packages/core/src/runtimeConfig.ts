...
export function createWispySmokeRuntimeConfig(
  ...
  return {
    ...
    fields: [
      {
        ...
        curlStrength: numberValue(values, "curlStrength", 2.6),
        id: "curl_field",
        scale: numberValue(values, "detailScale", 19),
        speed: numberValue(values, "detailSpeed", 0.65),
        strength: numberValue(values, "turbulence", 2.35),
        type: "curl",
        vorticityConfinement: numberValue(values, "vorticityConfinement", 1.8),
      },
    ],
    forces: [
      {
    ...
    solver: {
      advectionMode,
      backendMode,
      densityDissipation: numberValue(values, "densityDissipation", 0.085),
      ...
      velocityDissipation: numberValue(values, "velocityDissipation", 0.03),
    },
    sourceGlow: {
      blendMode: "additive",
  ...
  const fields = nodes
    ...
    .map((node) => {
      ...
      return {
        ...
        curlStrength: numberValue(values, "curlStrength", 2.6),
        id: node.id,
        scale: node.type === "field.fbm" ? numberValue(values, "detailScale", 19) : numberValue(values, "detailScale", 19),
        speed: numberValue(values, "detailSpeed", 0.65),
        strength: node.type === "field.fbm" ? numberValue(values, "detailStrength", 3.2) : numberValue(values, "turbulence", 2.35),
