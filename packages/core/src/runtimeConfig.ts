...
export function createWispySmokeRuntimeConfig(
  ...
  return {
    ...
    emitters: [
      {
        ...
        id: "emitter",
        lifetime: numberValue(values, "lifetime", 4.8),
        ...
        radius: numberValue(values, "radius", 0.36),
        scale: vec3Value(values, "sourceScale", [0.78, 0.34, 0.78]),
        shape: "sphere",
        spawnRate: numberValue(values, "spawnRate", 940),
        temperature: numberValue(values, "sourceTemperature", 1.28),
        velocity: vec3Value(values, "sourceVelocity", [0.02, 1.55, 0.02]),
      },
    ],
    fields: [
      {
    ...
    forces: [
      {
        buoyantLift: numberValue(values, "buoyantLift", 2.25),
        id: "buoyancy",
        position: [0, 0, 0],
        radius: 1,
        riseSpeed: numberValue(values, "riseSpeed", 2.35),
        strength: 1,
        type: "buoyancy",
    ...
    obstacles: [],
    render: {
      absorption: numberValue(values, "absorption", 0.72),
      baseDensity: numberValue(values, "baseDensity", 2.25),
      blendMode,
      ...
      detailStrength: numberValue(values, "detailStrength", 5.25),
      opacity: numberValue(values, "opacity", 0.94),
      opacityRamp,
      plumeTaper: numberValue(values, "plumeTaper", 0.74),
    ...
    solver: {
      advectionMode,
      backendMode,
      densityDissipation: numberValue(values, "densityDissipation", 0.045),
...
export function compileWispySmokeRuntimeConfig(
  ...
  const fields = nodes
    ...
    .map((node) => {
      ...
      return {
        bands: intValue(values, "turbulenceBands", 4),
        curlStrength: numberValue(values, "curlStrength", 6.35),
        id: node.id,
        scale: node.type === "field.fbm" ? numberValue(values, "detailScale", 26) : numberValue(values, "detailScale", 26),
        speed: numberValue(values, "detailSpeed", 1.18),
        strength: node.type === "field.fbm" ? numberValue(values, "detailStrength", 5.25) : numberValue(values, "turbulence", 2.6),
